...
| |____ ____) | |____   | | | |_| | (_) / /_
 ...
 Group Members:
    150120825 ONURCAN ISLER
    150118043 BERK KIRTAY
 */
...
void BFS(double minutes){
    ...
    while(frontier.size()){
        ...
        if(doWeStop(curr,stop)){ // Check if current node contains a goal state.
...
void DFS_Standard(double minutes){
    ...
    printBoard(frontier.top()->board);
    while(frontier.size()){
        shared_ptr<Node> curr = frontier.top(); // The strategy now is LIFO.
        frontier.pop();
        numberOfExpandedNodes++;
        
        if(numberOfExpandedNodes%100000==0){
            cout << numberOfExpandedNodes << endl;
        }
        
        if(doWeStop(curr,stop)){ // Same functions are used. Stop if the goal stated is reached.
...
int main(int argc, char **argv)
{
    ...
    while(1){
        ...
        size_t memsizemb = t_info.resident_size;
        memsizemb /= 1024;
        memsizemb /= 1024;
        return memsizemb;
    }
